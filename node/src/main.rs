...
async fn run(matches: &ArgMatches<'_>) -> Result<()> {
    ...
    let signature_service =
        SignatureService::new(keypair.secret);
    ...
    match matches.subcommand() {
        ...
        ("client", Some(sub_matches)) => {
            ...
            let transaction_timeout = sub_matches
            ...
            let default_metrics = format!("logs/client-{}-metrics", client_id);
            let metrics_path = sub_matches
                .value_of("metrics-file")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from(default_metrics));

            let metrics_handle = client::Client::spawn(
                client_id,
                committee,
                client_parameters,
                reply_addr,
            ...
            return wait_for_client_shutdown(metrics_handle, duration).await;
        }
...
async fn wait_for_client_shutdown(metrics: MetricsCollector, run_duration: Duration) -> Result<()> {
    enum ShutdownReason {
        DurationElapsed,
        Signal,
    }
